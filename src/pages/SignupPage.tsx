...
const SignupPage: React.FC = () => {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-r from-purple-600 via-pink-500 to-red-500 px-4">
      <form onSubmit={form.handleSubmit(onSubmit)} className="w-full max-w-sm bg-white p-6 md:p-8 rounded-lg shadow-lg" aria-labelledby="signup-form-title">
        {/* Title */}
        ...
        {/* Password */}
        <div className="mb-6">
          <InputField control={form.control} name="password" label="Password" placeholder="Enter your password" type="password" />
        </div>

        {/* Submit Button */}
        <button type="submit" disabled={isLoading} className={`w-full bg-indigo-600 text-white font-semibold py-2 px-4 rounded-md hover:bg-indigo-700 transition disabled:opacity-70 flex items-center justify-center`} aria-busy={isLoading}>
          {isLoading ? (
            <>
        ...
        {/* Error Message */}
        {form.formState.errors.email && <p className="text-red-500 text-sm text-center mt-4">{form.formState.errors.email.message}</p>}
      </form>
    </div>
  );
};
